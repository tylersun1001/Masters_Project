...
class iss():
    ...
    def sw_instr(self, r1: str, imm: str, r2: str):
        addr = (self.registers[int(r1, 16)] + int(imm, 16)) % 2**16
        self.store_addr = Converter.int2hex(addr, 4)
        self.store_data = Converter.int2hex(self.registers[int(r2, 16)], 4)
    ...
    def step(self):
        ...
        self.outfile.write(curr_instr + "\n" + Converter.int2hex(self.pc, 4) + "\n" + Converter.int2hex(self.i_id, 4) + "\n")
        self.record_state()
        if (opcode == "c"):
            self.outfile.write("store " + self.store_addr + "<=" + self.store_data + "\n")
        else:
            self.outfile.write("\n")
        if (self.eot):
